...
void TempMotifCounter::GetAllNeighbors(int node, TIntV& nbrs, PUNGraph graph) {
  nbrs = TIntV();
...
void TempMotifCounter::Count3TEdge3NodeStars(double delta, Counter3D& pre_counts,
  ...
  for (int c = 0; c < centers.Len(); c++) {
    // Gather all adjacent events
    int center = centers[c];
	// Vector of pairs <timestamp, idx of the edge with this timestamp in events>
    TVec<TIntPair> ts_indices;
	//Vector of pairs <nbr, direction in {0,1}> this correspond to the static directed edges connecting the center and its neighbors
    TVec<StarEdgeData> events;
